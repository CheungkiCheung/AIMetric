...
AIMetric 已经完成从平台骨架到准生产原型的第一阶段建设。
...
- `质量归因深化`：缺陷归因已从计数升级到对比率，可查看 `AI 参与需求缺陷率`、`AI 触达 PR 逃逸缺陷率`、`发布失败关联缺陷数`、`事故关联缺陷数`。
- `业务价值指标`：企业指标管线已新增 `critical_requirement_cycle_time`，支持基于关键需求优先级和上线时间计算关键需求周期。
